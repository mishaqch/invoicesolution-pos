  ...
  return (
    <div className="container section">
      <SectionHeading
        ...
        title="Pick the way you invoice"
        lead="Whether you sell over a counter or raise invoices from the office, there's a product shaped for you — both fully FBR-compliant."
      />
      <div className="grid gap-6 lg:grid-cols-2">
        {PRODUCTS.map((p, i) => (
          <Reveal key={p.tag} delay={i * 0.08}>
            <div className="group relative h-full overflow-hidden rounded-2xl border border-slate-100 bg-white p-7 shadow-card transition-all hover:-translate-y-1 hover:shadow-glow">
              <div className={cn("inline-flex rounded-full bg-gradient-to-r px-3 py-1 text-xs font-semibold text-white", p.accent)}>
                {p.tag}
              </div>
              <h3 className="h3 mt-4">{p.title}</h3>
              <p className="mt-2 text-ink-soft">{p.desc}</p>
              <ul className="mt-6 grid grid-cols-2 gap-3">
                {p.points.map(({ icon: Icon, label }) => (
                  <li key={label} className="flex items-center gap-2 text-sm text-ink-soft">
                    <Icon className="h-4 w-4 shrink-0 text-brand-600" /> {label}
                  </li>
                ))}
              </ul>
              <Link
                to={p.to}
                className="mt-7 inline-flex items-center gap-1.5 text-sm font-semibold text-brand-700 hover:gap-2.5"
              >
