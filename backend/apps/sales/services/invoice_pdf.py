...
def _draw_page_chrome(canvas: Canvas, doc) -> None:
    ...
    # Footer: hairline + page number + tagline.
    canvas.setStrokeColor(HAIR)
    canvas.setLineWidth(0.5)
    canvas.line(15 * mm, 14 * mm, page_w - 15 * mm, 14 * mm)
    canvas.setFont("Helvetica", 7)
    canvas.setFillColor(INK_SOFT)
    canvas.drawString(15 * mm, 9 * mm, "Generated by Pakistan POS · FBR Digital Invoicing")
...
def render_invoice_pdf(invoice: Invoice) -> bytes:
    ...
    # Try the user-saved logo first, then the historical placeholder name.
    assets_dir = Path(settings.BASE_DIR) / "apps" / "sales" / "assets"
    fbr_logo_path = next(
        (p for p in (assets_dir / "fbrLogo.png", assets_dir / "fbr_di_logo.png") if p.exists()),
    ...
    qr_img = (
        _b64_to_image(render_png_b64(qr_payload),
                      width_mm=SEAL_SIZE_MM, height_mm=SEAL_SIZE_MM)
        if qr_payload else None
    )
    ...
    # Trust statement at the very bottom.
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        '<font color="#9ca3af" size="7">'
        'This is a computer-generated invoice and does not require a signature. '
        'Verify by scanning the QR with the FBR Tax Asaan app.'
        '</font>',
        micro,
    ))
